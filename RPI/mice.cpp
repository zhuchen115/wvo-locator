...
void mice_update(void)
{
	...
	if(fd == -1)
	{
		LOG_PRINT(LOG_WARNGING,"Error in Opening %s",pDev);
		return;
	}
...
{
	pthread_t threadm;
	if(pthread_create(&threadm,NULL,(void *)&mice_update,NULL))
	{
		LOG_PRINT(LOG_ERROR,"Error in creating mouse measurement thread");
		return;
	}
