...
namespace robot_control{
	...
	ControllerPort::ControllerPort()
	{
		...
		syslog(LOG_INFO, "Initializing SPI interface\n");
		...
		if(spi_handle == -1)
		{
			...
			#endif
			syslog(LOG_ERR,"Error in opening SPI\n");
			abort();
		}
	...
	{
		syslog(LOG_INFO,"Moving Forward using speed %u with distance %u\n",speed,distance);
	...
	{
		syslog(LOG_INFO,"Moving Back using speed %u with distance %u\n",speed,distance);
	...
	int ControllerPort::Stop()
	{
		...
		syslog(LOG_INFO,"Emergency Stop\n");
	...
	int ControllerPort::QuerySensor(unsigned short id)
	{
		...
		if(len>4)
		{
			...
			#endif
			syslog(LOG_WARNING,"Length of Reply overflow! The length of message received is %d!\n",len);
			return 0;
		}
		
		memcpy(value.arr,btsRecv+3,len);
		
		syslog(LOG_INFO,"Query sensor id %d, value %d\n",id,value);
	...
	int ControllerPort::SetIRLight(unsigned char dc)
	{
		...
		syslog(LOG_INFO,"Setting IR light to dc %d\n",dc);
		return _spi_send_message(btsMsg,6);
	}
	...
	int ControllerPort::SetWLight(unsigned char dc)
	{
		...
		syslog(LOG_INFO,"Setting White light to dc %d\n",dc);
		return _spi_send_message(btsMsg,6);
	}
	...
	int ControllerPort::CamPause(unsigned char value)
	{
		...
		syslog(LOG_INFO,"Cam pause was set to  %d\n",btsMsg[2]);
		return _spi_send_message(btsMsg,6);
	}
	...
	int ControllerPort::CamFilter(unsigned char enable)
	{
		...
		syslog(LOG_INFO,"Camera IR filter is set to %d\n",btsMsg[2]);
		return _spi_send_message(btsMsg,6);
	}
