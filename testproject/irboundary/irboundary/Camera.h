...
namespace image_recognize {
	...
	public:
		...
		Mat CapCalibrateImage(int index = 0, Size sz = Size(1024, 768),int exposure = 700, bool gray = true);
		...
		/// \param exposure the exposure time default: 700
		/// \param gray Gray Scale image default: true. If capture an IR image, let it true
		Mat CapRawImage(int index = 0, Size sz = Size(1024, 768), int exposure = 700, bool gray = true);
	private:
