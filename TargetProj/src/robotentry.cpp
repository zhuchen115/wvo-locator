...
int main()
{
	...
	while (1)
	{
		...
		else //The target has more than 0.8m away
		{
            ...
            LOG_PRINT(LOG_INFO,"Real Move Distance: %f",movedist);
			...
			targetdist = dim.DistanceByLine(target2.height, target1.height, movedist);
            LOG_PRINT(LOG_DEBUG,"H1: %d, H2:%d",target1.height,target2.height);
			...
			if (target1.orientation == 0)
			{
				...
				double angle = atan(objectoffset/targetdist);

                LOG_PRINT(LOG_INFO,"The angle of the object is %lf.",angle);
